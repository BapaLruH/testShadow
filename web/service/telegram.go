...
// The structure type is capitalized to indicate that it can be accessed by other packages
...
func (s *TelegramService) GetsystemStatus() string {
	...
	status += fmt.Sprintf("System type:%s\r\n", runtime.GOOS)
	status += fmt.Sprintf("System architecture:%s\r\n", runtime.GOARCH)
	...
	if err != nil {
		...
		status += fmt.Sprintf("Running time:%s\r\n", common.FormatTime(upTime))
	}
	//xray version
	status += fmt.Sprintf("xray version:%s\r\n", s.xrayService.GetXrayVersion())
	//ip address
	var ip string
	...
	for _, inbound := range inbouds {
		...
		if inbound.ExpiryTime == 0 {
			status += fmt.Sprintf("Expiration time: indefinitely\r\n \r\n")
		} else {
			status += fmt.Sprintf("Expiration time:%s\r\n \r\n", time.Unix((inbound.ExpiryTime/1000), 0).Format("2006-01-02 15:04:05"))
		}
	}
	return status
}
...
/restart will restart xray,this command will not restart xray-ui
